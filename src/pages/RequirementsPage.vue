...
<template>
  <div class="page requirements-page">
    ...
    <section class="rule-summary-band"><div><span>专业学位课</span><strong>≥ {{ rules.degreeCredits }} 学分</strong></div><div v-if="rules.core"><span>学位课结构</span><strong>{{ rules.core }} 核心 + {{ rules.professional }} 专业</strong></div><div v-if="rules.doctorDegreeCount"><span>博士课程门数</span><strong>≥ {{ rules.doctorDegreeCount }} 门</strong></div><div><span>公共选修</span><strong>{{ rules.publicElective ? `≥ ${rules.publicElective} 学分` : '无硬指标' }}</strong></div><div><span>每学期</span><strong>≥ 10 有效学分</strong></div></section>
    ...
    <section class="source-note"><FileText :size="19" /><div><strong>计算口径</strong><p>核心课包含学科核心课和专业核心课；专业课仅按“专业课”属性统计。人文系列讲座、科学前沿讲座不计入每学期10学分。普博和工程博士按图片分类矩阵不设公共选修硬指标。</p></div></section>
  </div>
</template>
